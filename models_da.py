...
from DA.DA import GridDA
from DA.DA import grad_reverse
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), arc='default'):
        ...
        # init DA modules
        self.featmapDA_scale13 = GridDA(1024)
        self.featmapDA_scale26 = GridDA(512)
        self.featmapDA_scale52 = GridDA(256)
        self.featmapDA_img = GridDA(512)

    def forward(self, x, domain_label=None, var=None):
        img_size = x.shape[-2:]
        layer_outputs = []
        output = []
        # print(domain_label)
        ...
        conv_count = 0
        before_yolo_layer_num = 0
        before_yolo_layer_name = None
        layer_count = 0
        # DA output
        ret_da_pred = []
        ret_da_lab = []
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                ...
                if True:
                    if mtype == 'convolutional':
                        ...
                        if conv_count == 52:
                            ...
                            img_da_pred_map, img_lab_map = self.featmapDA_img(x_rev, domain_label)
                            ret_da_pred.append(img_da_pred_map)
                            ret_da_lab.append(img_lab_map)
                        elif conv_count == 60:
                            # scale 13, in dim 1024
                            x_rev = grad_reverse(x)
                            scale13_da_pred_map, scale13_lab_map = self.featmapDA_scale13(x_rev, domain_label)
                            ret_da_pred.append(scale13_da_pred_map)
                            ret_da_lab.append(scale13_lab_map)
                        elif conv_count == 68:
                            # scale 26, in dim 512
                            x_rev = grad_reverse(x)
                            scale26_da_pred_map, scale26_lab_map = self.featmapDA_scale26(x_rev, domain_label)
                            ret_da_pred.append(scale26_da_pred_map)
                            ret_da_lab.append(scale26_lab_map)
                        elif conv_count == 76:
                            # scale 52, in dim 256
                            x_rev = grad_reverse(x)
                            scale52_da_pred_map, scale52_lab_map = self.featmapDA_scale52(x_rev, domain_label)
                            ret_da_pred.append(scale52_da_pred_map)
                            ret_da_lab.append(scale52_lab_map)
                ...
                # total_layer: 89, 101, 113
                # last layer: conv
                # conv_count: 60, 68, 76
                # before_yolo_layer_num = layer_count
                layer_count += 1
        ...
        if self.training:
            ...
            return torch.cat(io, 1), p, ret_da_pred, ret_da_lab
        elif ONNX_EXPORT:
            ...
            io, p = list(zip(*output))  # inference output, training output
            return torch.cat(io, 1), p, ret_da_pred
